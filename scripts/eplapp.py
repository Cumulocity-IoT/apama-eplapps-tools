...
class EPLAppsCLI:
	...
	class Command:
		...
		def execute(self, optlist: list):
			...
			if printHelpRequested:
				...
				returnValue = self.function(eplApps, **self.arguments)
	...
	def printUsage(self, command=None):
		...
		if command in self.commands:
			...
			usageTable = [
				['Usage:', 'eplapp.py <command> <options>'],
				['', 'eplapp.py [--help]', 'eplapp.py [--version]']
			]
	...
	def printEPLAppsList(self, eplAppsJSON: list):
		"""
		Prints a formatted list of EPL apps.

		:param eplAppsJSON: A list of EPL apps in JSON form
		"""
		eplAppsString = '\nEPL apps:'
		if eplAppsJSON is None or len(eplAppsJSON) == 0:
			eplAppsString += '\nNo EPL apps to display.'
		else:
			for app in eplAppsJSON:
				eplAppsString += '\n'
				printMessage = [
					['name', app['name']],
