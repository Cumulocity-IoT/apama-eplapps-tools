...
class PySysTest(EPLAppsPerfTest):
	...
	def execute(self):
		...
		self.generateHTMLReport(f'{self.numDevices} devices with input rate of {self.inputRatePerDevice} eps/device', testConfigurationDetails=self.getTestConfigurationDetails(), 
	...
	def deploySampleApp(self):
		...
		self.waitForGrep(self.platform.getApamaLogFile(), expr='Added monitor eplfiles.PYSYS_AlarmOnMeasurementThreshold', errorExpr=['Error injecting monitorscript from file PYSYS_AlarmOnMeasurementThreshold'])
